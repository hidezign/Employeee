...
export default function EmployeeAttendance() {
  ...
  const fetchAttendance = async () => {
    try {
      const { data } = await axios.get(`${API_URL}/api/attendance/me`, { withCredentials: true });
      setAttendance(data);
    } catch (error) {
  ...
  return (
    <div className="flex">
      ...
      <div className="ml-64 flex-1 min-h-screen bg-[#FAFAFA]">
        <div className="p-6 md:p-8" data-testid="employee-attendance-page">
          <h1 className="text-4xl tracking-tighter font-black mb-2" data-testid="employee-attendance-title">
            MY ATTENDANCE
          </h1>
          <p className="text-sm text-zinc-500 mb-8">View your attendance history</p>

          <div className="bg-white border border-zinc-200">
            <table className="w-full" data-testid="employee-attendance-table">
              <thead>
              ...
              <tbody>
                {attendance.map((record) => (
                  <tr key={record._id} className="border-b border-zinc-100" data-testid={`attendance-row-${record._id}`}>
                    <td className="p-4 text-sm font-bold">{record.date}</td>
